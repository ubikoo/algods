...
#ifndef NUMERIC_SDE_STRAT_H_
#define NUMERIC_SDE_STRAT_H_

#include <type_traits>
#include "algods/numeric/array.h"

///
/// Solve the set of sdes of the specified type using stratonovich stochastic
/// integrator:
...
///     <[dw]^2> = dt
///
/// The integration step uses the Stratonovich interpration of stochastic
/// differential equation(implicit Heun Method):
///
...
struct SdeStrat {
    ...
    SdeStrat(SdeStrat &&other)
        : wiener_(std::move(wiener_))
        , neq_(std::move(neq_))
        , dw_(std::move(other.dw_))
        , y_(std::move(other.y_)) {}

    SdeStrat &operator=(SdeStrat &&other) {
        if (this == &other) {
            return *this;
        }
        wiener_ = std::move(wiener_);
