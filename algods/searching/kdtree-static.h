...
#ifndef SEARCHING_KDTREE_STATIC_H_
#define SEARCHING_KDTREE_STATIC_H_
...
///
/// @note KdTreeStatic underlying data structure is represented by a vector of
/// nodes held by a unique pointer. The data structure is static and the class
/// does not allow removal of node points from an existing tree.
/// This makes the static variant a more efficient data structure to perform
/// search queries over a collection of points.
///
...
class KdTreeStatic {
    ...
    static_assert(std::is_floating_point<Type>::value,
        "non floating point numeric type");
...
public:
    ...
    // Insert a point represented by its coordinates into the kd-tree. Do nothing
    ...
    // Return all the points in the tree by running a range query over an axis
    // aligned bounding box given by the lowest and largest values for the
    // floating type.
    ...
    size_t cutdim)
{
    ...
    // If the point coordinate along the node cutting dimension is less than
    // the node point coordinate, insert the point on the left subtree.
    ...
    size_t dim) const
{
    ...
    // If the node cutting dimension matches the search cutting dimension, we
    // only need to search on the right subtree. If the node is a leaf, we found
    // the maximum. Otherwise, search along its right subtree.
    // If the node cutting dimension does not match the search cutting dimension,
    ...
    queue_type &result) const
{
    ...
    // If the queue is full, check whether the new node needs to be added.
    // If the new distance is smaller than the largest distance in the queue (top
    //  node), pop the top node and push the new node.
...
///
/// @brief Return the minimum Euclidean distance from q to any point on the
/// rectangle. Compute the square root sum of the distance from coordinate
...
/// Return the subrectangle left of the hyperplane at point and normal to the
/// cut dimension dim. Formaly, return a subrectangle whose low point is lo and
/// whose high point is hi, except for the dim-th coordinate, given by q[dim].
///
KDTREE_T_DECL
...
/// Given a cutting dimension dim and a point lying within the rectangle.
/// Return the subrectangle right of the hyperplane at point and normal to the
/// cut dimension dim. Formaly, return a subrectangle whose high point is hi and
/// whose low point is low, except for the dim-th coordinate, given by q[dim].
///
KDTREE_T_DECL
...
#endif // SEARCHING_KDTREE_STATIC_H_
