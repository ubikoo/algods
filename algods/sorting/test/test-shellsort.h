...
#ifndef TEST_SHELLSORT_H_
#define TEST_SHELLSORT_H_
...
void test_shellsort(size_t n)
{
    ...
    // Test Sort
    Sort<T, std::less_equal, ShellSort>(v);
    for (size_t i = 0; i < v.size()-1; ++i) {
        REQUIRE(v[i] <= v[i+1]);                // sort in decreasing order
    }

    Sort<T, std::greater_equal, ShellSort>(v);
    for (size_t i = 0; i < v.size()-1; ++i) {
